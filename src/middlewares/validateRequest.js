...
const validateRequestBody = (requiredFields) => {
  return (req, res, next) => {
    ...
    if (missingFields.length > 0) {
      throw new AppError(CommonError.INVALID_INPUT, `필수 입력값이 누락되었습니다: ${missingFields.join(', ')}`, 400);
    }

    if (unexpectedFields.length > 0) {
      throw new AppError(CommonError.INVALID_INPUT, `유효하지 않은 입력이 포함되었습니다.`, 400);
    }
